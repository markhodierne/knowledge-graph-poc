...
def get_lesson_plans_by_id(sample_id, limit=None):
    ...
    Returns:
        pd.DataFrame: DataFrame of lesson plans with column names.
    """
    query = """
        SELECT lp.id AS lesson_plan_id, lp.lesson_id, lp.json AS lesson_json_str
    ...
    # Fetch the results
    results = execute_single_query(query, params)  # still returns a list of tuples

    # Convert to DataFrame
    columns = ["lesson_plan_id", "lesson_id", "lesson_json_str"]
    lesson_plans_df = pd.DataFrame(results, columns=columns)

    return lesson_plans_df
