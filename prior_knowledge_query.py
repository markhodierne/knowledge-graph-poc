...
# --- Streamlit UI ---
st.set_page_config("Prior knowledge Query", page_icon="📘")
st.title("Prior knowledge Query")
...
# The key fix: Use a session state key to track the dropdown selection
...
# Create a callback function that will update the session state when selection changes
def on_lesson_selection_change():
    # This will run when the selectbox value changes
    st.session_state.lesson_selection = st.session_state.lesson_selectbox

# Determine the correct index based on session state
if st.session_state.lesson_selection in lesson_options:
    ...
    # If selection not in options (e.g., data changed), reset to first
    default_index = 0
    st.session_state.lesson_selection = list(lesson_options.keys())[0] if lesson_options else None

# Render the dropdown with the callback
selected_preview = st.selectbox(
    "Select a lesson plan to query:",
    options=list(lesson_options.keys()),
    index=default_index,
    key="lesson_selectbox",  # This key connects to the session state
    on_change=on_lesson_selection_change  # This triggers the callback when changed
)

st.subheader("🔎 Find Concepts and Prior Knowledge from Input Text")

selected_lesson_plan = lesson_options[selected_preview]
top_k = st.slider("How many top concepts to match?", 1, 10, 5)
hop_limit = st.slider("Max prerequisite depth (hops)", 1, 5, 2)
min_sim = st.slider("Minimum similarity threshold", 0.0, 1.0, 0.6, 0.05)

if st.button("Find Matching Concepts and Prior Knowledge"):
    with st.spinner("Querying knowledge graph..."):
        top_matches = find_most_similar_concepts(selected_lesson_plan, top_k=top_k, min_similarity=min_sim)

        if not top_matches:
            st.warning("❗ No concepts matched above the similarity threshold.")
        else:
            st.markdown("### 🔍 Most Similar Concepts")
            for concept_name, score in top_matches:
                lesson_title = get_most_relevant_lesson_for_concept(concept_name)
                st.markdown(f"**{concept_name}** (similarity: {score:.3f}) — Taught in: _{lesson_title}_")

                prereqs = get_prerequisite_concepts(concept_name, max_hops=hop_limit)
                if prereqs:
                    st.markdown(f"**Prerequisites for {concept_name}:**")
                    prereq_df = pd.DataFrame(prereqs)
                    st.dataframe(prereq_df)
                else:
                    st.warning(f"No prerequisites found for {concept_name}.")
