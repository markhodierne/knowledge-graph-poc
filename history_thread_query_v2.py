...
def generate_statements(df: pd.DataFrame, model_name: str = "gpt-4o") -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        current_board = row['examBoardTitle']
        ...
        row_group = pd.DataFrame([row])
        prompt = construct_prompt(row_group, prior_progression=prior_progression)
        ...
        # Attach results to row
        row_out = row.copy()
        row_out["threadProgressionStatement_para2"] = completion
        row_out["threadProgressionContext"] = prior_progression
...
if __name__ == "__main__":
    ...
    # Step 2: Drop duplicates based on paragraph 1
    deduped_df = enriched_df.drop_duplicates(subset=["threadProgressionStatement_para1"])

    # Step 3: Sort the units before passing them to the LLM
    deduped_df = deduped_df.sort_values(by=['threadTitle', 'yearGroup', 'unitOrder']).reset_index(drop=True)

    # Step 4: Assign unitThreadOrder
    deduped_df['unitThreadOrder'] = range(1, len(deduped_df) + 1)

    # Step 5: Generate LLM completions
    final_df = generate_statements(deduped_df)

    # Step 6: Concatenate paragraph 1 and 2
    ...
    columns_to_keep = [
        ...
        'threadProgressionContext'  # DEBUG line
    ]
