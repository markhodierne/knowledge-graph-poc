"""
CAT Data Import Module (Unit content)
...
st.set_page_config("Import Unit Data", page_icon="😺")
st.title("Import CAT Unit Data to CSV")
...
class Config:
    ...
    CAT_DATA_CSV: Path = DATA_DIR / 'cat_unit_data.csv'
...
class DataProcessor:
    ...
    def unify_column_types(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in json_cols:
            df[col] = df[col].apply(
                lambda x: (
                    json.dumps(x) if isinstance(x, (list, dict))
...
class CATDataImporter:
    ...
    def extract_id(self, value):
        """Extract the 'id' from a JSON string containing a list of dictionaries."""
        try:
            if isinstance(value, str):  # Check if it's a string
                value = json.loads(value)  # Parse the JSON string into a Python object
            if isinstance(value, list) and len(value) > 0 and isinstance(value[0], dict):
                return value[0].get('id')  # Extract the 'id' from the first dictionary
        except json.JSONDecodeError as e:
            st.write("JSON Decode Error:", e)

        return None  # Return None if no valid data
    
    def import_data(self ) -> Optional[pd.DataFrame]:
        """Import CAT unit data."""
        if not self.client:
        ...
        # Extract IDs from JSON columns
        for col in ['futureUnit', 'priorUnit']:
            if col in df.columns:
                df[col] = df[col].apply(self.extract_id)
        
        # Add subject column
        df2 = pd.read_csv("data/cat_data.csv")
        df = df.merge(df2[['unitId', 'subjectTitle']], on="unitId", how="left")
        df = df.dropna(subset=['subjectTitle'])
        
        return df

    def _query_data(self) -> List[Dict]:
        """Query data from Hasura."""
        query = self._build_unit_query()
        response = self.client.execute_query(query)
        
        return (response.get('data', {})
                .get('published_mv_openapi_unit_curriculum_content_1_0_2', []))

    @staticmethod
    def _build_unit_query() -> str:
